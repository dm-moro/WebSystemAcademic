...
case "alerta":
  formHTML += `
    ...
    <div class="form-group">
      <label for="statusFiltro">Filtrar por status de pagamento:</label>
  ...
  setTimeout(() => {
    ...
    if (statusFiltro) {
      statusFiltro.addEventListener('change', filtrarAlertasPorStatus);
      filtrarAlertasPorStatus();
    }
  }, 100);
  return formHTML;
  break;
...
async function carregarAlertasEnviados() {
  ...
  function filtrarAlertasPorStatus() {
    ...
    if (!statusSelecionado) {
      listaDiv.innerHTML = '<p>Selecione um status para exibir os alunos.</p>';
      return;
    }

    let pagamentosFiltrados = currentData.pagamentos.filter(p => p.status === statusSelecionado);
    ...
    if (pagamentosFiltrados.length === 0) {
      listaDiv.innerHTML = '<p>Nenhum pagamento encontrado com esse status.</p>';
      return;
    }

    let html = '<ul style="list-style:none;padding:0;">';
    pagamentosFiltrados.forEach(pagamento => {
      const matricula = currentData.matriculas.find(m => m.id_matricula === pagamento.id_matricula);
      const aluno = currentData.alunos.find(a => a.id_aluno === matricula?.id_aluno);
